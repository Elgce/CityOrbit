...
parser.add_argument("--task", type=str, default="Isaac-Velocity-Flat-Unitree-Aliengo-v0", help="Name of the task.")
...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # agent stepping
            if get_data:
                obs[:, 9:12] = torch.tensor([0.0, 0.0, 0.0], device=env.device, dtype=torch.float)
            ...
            position_list.append(obs[:, 12:24].cpu().numpy())
            velocity_list.append(obs[:, 24:36].cpu().numpy())
